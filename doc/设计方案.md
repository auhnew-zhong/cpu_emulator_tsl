...
- 2字节：`trigger_pos(0x4)`、`jmp(0x5)`、`bl(0x9)`、`send(0xB)`、`edge_detect(0xE)`、`domain_set(0xA)`、`mov(0x7, func=1)`
...
- `send`：统一发送内建操作，包含 display 和 exec 等（`src/cpu.c:556-590`）
- `dump_registers`：额外打印计数器/定时器/PC，便于时序与控制流调试（`src/cpu.c:755-761`）

### 3.5 信息表与定时器
信息表接口：📗 `include/info_db.h`
- `get_complete_display_string`、`get_exec_info`、`get_domain_info`
- `timer_tick_and_jump`（驱动阈值与跳转目标，`src/info_db.c`）
